...
export const mockKoordinatorsDeltakerliste = (
...
export const mockGjennomforinger: MockGjennomforing[] = lagMockGjennomforinger(
  gjennomforingInfoListe
)
...
export const lagMockTiltakDeltakere = (
  ...
  const deltakereJobbklubb = lagMockTiltakDeltagereForGjennomforing(
    mockGjennomforinger[8],
    40
  )
  const deltakereGruppeAmoNordvest = lagMockTiltakDeltagereForGjennomforing(
    mockGjennomforinger[9],
    10
  )
  const deltakereGruppeAmoSorvest = lagMockTiltakDeltagereForGjennomforing(
    mockGjennomforinger[10],
    15
  )
  ...
  return tilfeldigeDeltakere.concat(
    deltakereJobbklubb,
    deltakereGruppeAmoNordvest,
    deltakereGruppeAmoSorvest,
    deltakereGruppeFagOgYrkeSorvest
  )
}

const deltakereTilVeileder = () => {
  const g1 = mockTiltakDeltakere[0].gjennomforing.id
  const g2 = mockTiltakDeltakere.find((d) => d.gjennomforing.id !== g1)
    ?.gjennomforing.id

  const deltakere1 = mockTiltakDeltakere
    .filter((d) => d.gjennomforing.id === g1)
    .slice(0, 10)
  const deltakere2 = mockTiltakDeltakere
    .filter((d) => d.gjennomforing.id === g2)
    .slice(0, 5)
...
export const mockMineDeltakerlister: MineDeltakerlister =
  lagMockMineDeltakerlister(mockGjennomforinger, mockDeltakerlisteVeileder)
