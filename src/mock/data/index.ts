...
const deltakereTilVeileder = () => {
	const g1 = mockTiltakDeltakere[0].deltakerliste.id
	const g2 = mockTiltakDeltakere.find(d => d.deltakerliste.id !== g1)?.deltakerliste.id

	const deltakere1 = mockTiltakDeltakere.filter(d => d.deltakerliste.id === g1).slice(0, 10)
	const deltakere2 = mockTiltakDeltakere.filter(d => d.deltakerliste.id === g2).slice(0, 5)
