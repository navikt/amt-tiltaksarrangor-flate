...
export const lagMockEndringsmeldingForDeltaker = (
  ...
  if (
    n >= 1 &&
    n <= 2 &&
    typeof deltakerStatus === typeof TiltakDeltakerStatus.VENTER_PA_OPPSTART
  ) {
    return [
      {
    ...
    return [
      {
        ...
        innhold: {
          ...
          aarsak: { type: DeltakerStatusAarsakType.SYK, beskrivelse: null }
        },
        sendt: faker.date.recent(),
        status: EndringsmeldingStatus.AKTIV
      }
  ...
  if (
    n < 3 &&
    typeof deltakerStatus === typeof TiltakDeltakerStatus.HAR_SLUTTET
  ) {
    return [
      {
        id: endringsmeldingId(),
...
export const lagMockHistoriskeEndringsmeldingForDeltaker = (
  ...
  if (
    n < 4 &&
    typeof deltakerStatus === typeof TiltakDeltakerStatus.IKKE_AKTUELL
  ) {
    return [
      {
        id: endringsmeldingId(),
