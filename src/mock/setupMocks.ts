...
import {
  ulestEndringErNyeDeltaker,
  ulestEndringErOppdateringFraNav,
  ulestEndringErSvarFraNav
} from '../component/page/bruker-detaljer/deltaker-detaljer/forslag/forslagUtils'
...
export const worker = setupWorker(
  ...
  http.post(
    ...
    async ({ params }) => {
      ...
      deltaker.svarFraNav = deltaker.ulesteEndringer.find(
        ulestEndringErSvarFraNav
      )
        ? true
        : false
      deltaker.oppdateringFraNav = deltaker.ulesteEndringer.find(
        ulestEndringErOppdateringFraNav
      )
        ? true
        : false
      deltaker.nyDeltaker = deltaker.ulesteEndringer.find(
        ulestEndringErNyeDeltaker
      )
        ? true
        : false
  ...
  http.get(appUrl('/amt-tiltaksarrangor-bff/unleash/api/feature'), async () => {
    await delay(500)
    const toggles = {
