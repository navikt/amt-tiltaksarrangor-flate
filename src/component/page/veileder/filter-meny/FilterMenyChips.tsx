...
import { mapTiltakDeltakerStatusTilTekst, mapVeilderTypeTilTekst } from '../../../../utils/text-mappers'

export const FilterMenyChips = (): React.ReactElement => {
	const {
		statusFilter, veiledertypeFilter, deltakerlisteFilter,
		removeStatusFilter, removeVeilederTypeFilter, removeDeltakerlisteFilter
	} = useVeilederFilterMenyStore()

	return (
		<Chips className={globalStyles.blokkXs} title="Fjern filter">
			{statusFilter.map(status => <Chips.Removable
				key={status}
				variant="action"
			...
			{deltakerlisteFilter.map(deltakerListe => <Chips.Removable
				key={deltakerListe}
				variant="action"
			...
			{veiledertypeFilter.map(veilederType => <Chips.Removable
				key={veilederType}
				variant="action"
