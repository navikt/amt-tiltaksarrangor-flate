...
export const DeltakerlistePanel = (props: DeltakerlistePanelProps): React.ReactElement<DeltakerlistePanelProps> => {
	const { id, navn, startdato, sluttdato } = props
		...
		<li className={globalStyles.blokkS}>
			<SpaLenkepanel to={deltakerlisteDetaljerPageUrl(id)}>
				<div className={styles.content} >
					...
					{(startdato || sluttdato) && <BodyShort size="small" className={styles.datotekst}><span>{formatDate(startdato)} - {formatDate(sluttdato)}</span></BodyShort>}
				</div>
			</SpaLenkepanel>
		</li>
	)
