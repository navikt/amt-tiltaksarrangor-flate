...
export function validerAarsakForm(aarsak?: DeltakerStatusAarsakType, beskrivelse?: string | null, begrunnelse?: string): Promise<ValidertAarsakForm> {
	return new Promise((resolve, reject) => {
		if (!aarsak) {
			return reject('Årsak er påkrevd')
		}
		if (aarsak === DeltakerStatusAarsakType.ANNET && !beskrivelse) {
			return reject(`Beskrivelse er påkrevd med årsak '${aarsakTekstMapper(aarsak)}'`)
		}
		if (aarsak === DeltakerStatusAarsakType.ANNET && beskrivelse && beskrivelse.length > 40) {
			return reject(`Beskrivelse kan ikke være mer enn 40 tegn med årsak '${aarsakTekstMapper(aarsak)}'`)
		}
		if (begrunnelse && begrunnelse.length > BEGRUNNELSE_MAKS_TEGN) {
			return reject(`Begrunnelse kan ikke være lengre enn ${BEGRUNNELSE_MAKS_TEGN} tegn`)
		}
		return resolve({
			endringsmelding: {
				aarsak: {
...
export function useAarsakValidering(aarsak?: DeltakerStatusAarsakType, beskrivelse?: string | null, begrunnelse?: string) {
	const [ validering, setValidering ] = useState<ValideringState>({ isSuccess: false, feilmelding: null })
...
function toEndringAarsakType(aarsak: DeltakerStatusAarsakType, beskrivelse: string | undefined | null): EndringAarsak {
	switch (aarsak) {
		...
		default: throw new Error(`Kan ikke konvertere ${aarsak} til EndringsAarsakType`)
	}
}
