...
const getHistoriskEnrdingsmeldingPanel = (
  historiskEndringsmelding: Endringsmelding
) => {
  let statusInfo = null
  if (historiskEndringsmelding.status === EndringsmeldingStatus.TILBAKEKALT) {
    statusInfo = 'Arrangør tilbakekalte meldingen.'
  } else if (
    historiskEndringsmelding.status === EndringsmeldingStatus.UTDATERT
  ) {
  ...
  return (
    <div className={styles.panel}>
      <EndringTypeIkon
        type={mapTilEndringType(historiskEndringsmelding.type)}
      />
      <div>
    ...
    >
      <ul className={styles.list}>
        {historiskeEndringsmeldinger.map((e) => (
          <li key={e.id}>{getHistoriskEnrdingsmeldingPanel(e)}</li>
        ))}
      </ul>
    </ReadMore>
  )
