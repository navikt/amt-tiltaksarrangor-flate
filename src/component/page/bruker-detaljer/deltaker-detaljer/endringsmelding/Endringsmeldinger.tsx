...
export const Endringsmeldinger = ({
  ...
  const [historiskeEndringsmeldinger, setHistoriskeEndringsmeldinger] =
    useState<Endringsmelding[]>(deltaker.historiskeEndringsmeldinger || [])
  ...
  return (
    <div className={styles.endringsmeldinger}>
      {visfeilmelding && (
        <Alert variant="error">Kunne ikke hente endringsmeldinger</Alert>
      )}
      ...
      {historiskeEndringsmeldinger.length > 0 && (
        <HistoriskeEndringsmeldinger
          historiskeEndringsmeldinger={historiskeEndringsmeldinger}
        />
      )}
    </div>
  )
