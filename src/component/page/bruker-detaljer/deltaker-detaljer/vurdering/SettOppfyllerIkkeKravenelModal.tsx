...
export const SettOppfyllerIkkeKravenelModal = ({ isOpen, deltakerId, onClose, onVurderingSendt }: OppfyllerIkkeKravenelModalProps) => {
	const [ beskrivelse, settBeskrivelse ] = useState<Nullable<string>>()

	const sendVurdering = () => {
		if (!beskrivelse) {
			return Promise.reject('Beskrivelse er påkrevd for vurdering: Oppfyller ikke kravene')
		}
	...
	const kanSendeVurdering = beskrivelse && beskrivelse.length <= BESKRIVELSE_MAKS_TEGN

	return (
		<Modal open={isOpen} onClose={onClose} aria-labelledby="modal-heading" header={{ heading: 'Oppfyller ikke kravene' }}>
			<Modal.Body className={styles.content}>
				<BodyShort size="small">Personen oppfyller ikke kravene for å delta på dette arbeidsmarkedstiltaket.</BodyShort>
				<Textarea
					...
					size="small"
				/>
				<Button type="button" onClick={sendVurdering} className={styles.modalKnapp} disabled={!kanSendeVurdering} size="small">
					Send til NAV
				</Button>
			</Modal.Body>
		</Modal>
